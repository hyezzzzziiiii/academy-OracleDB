-- 오라클 명령어 그룹함수
-- Internal Function 들의 연산대상이 필드 또는 레코드 였다면
-- 그룹함수는 테이블 전체(각 필드값 전체)를 대상으로 하는 함수를 칭합니다.
-- 학생들의 성적(필드명 : 이름 국어 영어 수학)을 저장하고 있는 테이블에서 과목 총점을 내거나
-- 과목 평균을 내야하는 경우, 또는 특정조건에 의해 필터링된 그룹들의 총점 평균들을 게산할 때 사용합니다.

-- 일반적인 한 명의 학생의 총점 : select 국어 + 영어 + 수학 as 총점 from 성적;
-- => 학생 수만큼의 결과가 출력
-- 그룹함수를 이용한 과목 총점 : select sum(국어) as 국어과목총점 from 성적 
-- => 결과가 한개 출력
...
-- 직업별 급여 평균 + 급여 평균이 2000이상인 데이터만 표시)
select job, round(avg(sal),2) as 평균급여
from emp
group by job
having avg(sal)>2000;
