...
-- - 뷰는 기본테이블에서 파생 내지는 가공된 객체로서 기본테이블에 대한 하나의 쿼리(질의, 명령)문입니다
...
-- 시퀀스는 .nextVal 메서드가 실행되는 순간 숫자가 증가합니다. 그래서 추후 웹프로젝트 에서 실습할때
-- 다른 요인에 의한 실행 실패에도 .nextVal 가 실행된 상태라면 숫자는 증가된 상태로 종료하게 됩니다
-- 위의 예제에서도 select 구문 안에서 memos_seq.nextVal 가 실행될때 마다 숫자는 증가하게 됩니다.
