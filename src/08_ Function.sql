...
select * from dual; -- 라이브러리 함수의 결과를 보기위한 키워드 사용 dual :임시 데이터 출력시 사용합니다
-- 임시 데이터란 특정 테이블을 대상으로 하는 select 문이 아니면서 계산의 결과가 출력되어야 하는 곳에 쓰는 
-- 키워드입니다

--[2]임시 데이터 출력
select sal*12+nvl(comm, 0) as "연봉" from emp; --emp 테이블 대상
select 1234*1234 from dual; -- 특정테이블이 대상이 아닌 연산결과 조회
...
-- *** 날짜 처리 관련 함수 ***
--[17] sysdate : 날짜
select sysdate from dual; -- 오늘 날짜와 현재 시간 2020-12-17 17:10:52.0

--[18] months_between(): 개월 수 구하기
select months_between( '2021-12-31', '2020-07-10') from dual; --17.677419... 개월수

--[19] add_months() : 개월 수 더하기 
select add_months(sysdate, 200) from dual; --2037-08-17 17:13:38.0

--[20] next_day() : 다가올 요일에 해당하는 날짜 - 오늘 날짜(sysdate)에서 가장 가까운 일요일
select next_day(sysdate, '일요일') from dual; --2020-12-20 17:15:14.0

--[21] last_day() : 해당 달의 마지막 일 수
select last_day(sysdate) from dual; -- 2020-12-31 17:15:14.0
select last_day('2020-12-15') from dual;

--[22] to_char() : 문자열로 반환
select to_char(sysdate, 'yyyy-mm-dd') from dual; --2020-07-10
...
select ename, deptno, decode( deptno, 10, 'ACCOUNT',
						...
						80, 'SA_REP',					
						90, 'AD_PRES',					
						100, 'FI_ACCOUNT', 110, 'AC_ACCOUNT' ) as "부서이름" from emp; 

-- deptno 값이 무엇이냐에 따라 부서이름이 결정되어 결과 표시;
-- 부서번호(deptno) 필드와 부서명(dname) 필드가 정의되어 있는 테이블이 있다면 두개의 테이블을
-- join 하여 표시하면 되지만, 위의 경우 해당 테이블이 없을때 사용하는 방법 입니다.
​
